...
function buildUpdateHtml(d){
  ...
  return '<div style="max-width:640px;margin:0 auto;font-family:Arial,Helvetica,sans-serif;color:#141925">'
    + '<div style="background:linear-gradient(135deg,#4f46e5,#9333ea);color:#fff;padding:22px;border-radius:14px 14px 0 0">'
    ...
    + '</div>'
    + '<div style="padding:18px 2px">'+cards+'</div>'
...
async function emailBuildUpdate(id, btn){
  ...
  try {
    const res = await api('POST', '/api/email', { to, subject: 'Build Update — ' + d.name, text: buildUpdateText(d), html: buildUpdateHtml(d) });
